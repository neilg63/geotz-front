...
export interface DegreesMinutesSecondsStrings {
  deg: string;
  min: string;
  sec?: string;
  polarity?: string;
...
export interface CoordsDisplay {
  lat: DegreesMinutesSecondsStrings,
  lng: DegreesMinutesSecondsStrings,
}
...
// @param flDeg:number
// @return Object
export const decDegToDmsStrings = (flDeg: any, axis = 'lat'): DegreesMinutesSecondsStrings => {
  const dms = decDegToDms(flDeg);
  const isLat = axis === 'lat';
  return { 
    deg: Math.abs(dms.deg).toFixed(0) + 'º',
    min: Math.abs(dms.min).toFixed(0) + `'`,
    sec: dms.sec ? Math.abs(dms.sec).toFixed(0) + `"` : '',
    polarity: dms.deg < 0 ? isLat ? 'S' : 'W' : isLat ? 'N' : 'E'
  }
};
...
export const toThousands = (integer: number) => {
  const str = Math.floor(integer).toString()
  const ln = str.length;
  if (ln > 3) {
    const parts = [];
    const numParts = Math.ceil(ln / 3);
    const firstGroupLen = ln % 3;
    const hasFirstGroup = firstGroupLen > 0;
    const numThGroups = hasFirstGroup ? numParts - 1 : numParts;
    if (hasFirstGroup) {
      parts.push(str.substring(0, firstGroupLen));
    }
    for (let i = 0; i < numThGroups; i++) {
      const startIndex = firstGroupLen + (i * 3);
      parts.push(str.substring(startIndex, startIndex + 3));
    }
    return parts.join(' ')
  } else {
    return str;
...
const correctStringNum = (val: string): number => {
...
export const inputToNumber = (val: any = null): number => {
  const dType = typeof val;
  switch (dType) {
    case 'number':
      return val;
    case 'string':
      return correctStringNum(val)
    default:
      return 0;
  }
}
