...
export class JulDate {
  ...
  get unixTime(): number {
    return Math.floor(julToUnixTime(this.jd));
  }
  ...
  get jdRound(): string {
    return toThousands(Math.round(this.jd));
  }

  get unixTimeDisplay() {
    return toThousands(this.unixTime);
  }

  get jdDecimals(): string {
    const places = 5;
    const div = Math.pow(10, places);
    const dec = this.jd % 1;
    return (Math.round(dec * div) / div).toFixed(places).split('.').pop() as string;
  }
...
export const currentJulDateOffset = (offset = 0) => {
  const jd = currentJulianDay();
  return new JulDate(jd, offset);
};
...
export const utcDateString = (ts: any = null): string => {
  const dt = typeof ts === 'number' ? new Date(ts) : new Date();
  const dtStr = dt.toISOString();
  return typeof dtStr === "string"? dtStr.split('.').shift() as string : "";
}

export const utcDateStringFromUnix = (ts = 0): string => {
  return utcDateString(ts * 1000);
}

export const currentUtcDateString = (): string => {
  return utcDateString(null);
}

export const localOffsetSecs = (): number => {
  return 0 - new Date().getTimezoneOffset() * 60;
}


export const fetchDateComponents = (hasTs = false, ts = 0) => {
  const dtObj1 = hasTs ? new Date(ts * 1000) : new Date();
  const msOffset = dtObj1.getTimezoneOffset() * 60 * 1000;
  const dtObj = new Date(dtObj1.getTime() - msOffset);
  const dt = dtObj.toISOString().split(".").shift() as string;
  const parts = dt.split("T");
  const date = parts.shift() as string;
  const time = parts.pop() as string;
  return { date, time, dt, msOffset }
}

export const fetchCurrentDateComponents = () => {
  return fetchDateComponents(false);
}

export const fetchRelativeDateComponents = (ts = 0) => {
  return fetchDateComponents(true, ts);
}


export const unixTimeToJul5 = (ts = 0): number => {
  const m = Math.pow(10, 5);
  return Math.round(unixTimeToJul(ts) * m) / m;
}

export const unixTimeToJulDate = (ts = 0, offset = 0): JulDate => {
  return new JulDate(unixTimeToJul(ts), offset)
}

export const roundFloat5 = (jd = 0) => {
  const m = Math.pow(10, 5);
  return Math.round(jd * m) / m;
}
